...
const BASE_URL = process.env.NEXT_PUBLIC_STARGATE_BASE_URL ?? 'https://stargate.finance/api/v1'
...
export async function fetchStargateNetworks(): Promise<StargateNetwork[]> {
  ...
  return list
    .map((entry) => {
      ...
      const rpcUrls = getStringList(record, ['rpcUrls', 'rpc', 'publicRpcUrls'])
      const explorer = getString(record, ['explorerUrl', 'scanUrl', 'blockExplorer']) ?? ''
      const nativeSymbol =
        getString(record, ['nativeSymbol', 'nativeTokenSymbol', 'gasSymbol']) ?? 'NATIVE'
    ...
    .filter((item): item is StargateNetwork => Boolean(item))
}
...
export async function fetchStargateTokens(params: {
  ...
  for (const entry of list) {
    ...
    const record = entry as Record<string, unknown>
    const address = getString(record, ['address', 'tokenAddress', 'contractAddress'])
    const symbol = getString(record, ['symbol'])
    const name = getString(record, ['name'])
    const decimals = getNumber(record, ['decimals'])
    const logoURI = getString(record, ['logoURI', 'logoUrl', 'icon']) ?? undefined
    const tokenChainKey = getString(record, ['chainKey'])

    if (!address || !symbol || !name || decimals === null || !tokenChainKey) continue
    if (tokenChainKey.toLowerCase() !== params.chainKey.toLowerCase()) continue

    const lowered = address.toLowerCase()
    if (lowered === 'native' || lowered === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee') {
      tokens.push({
